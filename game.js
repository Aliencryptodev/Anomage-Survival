...
// ===== MINI MAPA SIMPLE PARA TU JUEGO ACTUAL =====
// Añadir al final de game.js, después de boot();

/* =========================================================
   MINI MAPA SIMPLE Y FUNCIONAL
   ========================================================= */
...
class SimpleMiniMap {
  constructor() {
    ...
    this.ctx = null;
    this.visible = true;
    this.worldRange = 800; // Área del mundo que mostramos
    this.init();
  }
  ...
  init() {
    ...
    console.log('Mini mapa simple inicializado');
  }

  setupControls() {
    document.addEventListener('keydown', (e) => {
      if (e.key.toLowerCase() === 'm' && GAME.running) {
        this.toggle();
      }
    });
  }
  ...
  toggle() {
    ...
    toast(this.visible ? 'Mini mapa ON' : 'Mini mapa OFF');
  }
  ...
  worldToMinimap(worldX, worldY) {
    ...
    const scale = this.size / this.worldRange;
    
    const relativeX = worldX - GAME.player.x;
    const relativeY = worldY - GAME.player.y;
  ...
  drawPlayer() {
    ...
    ctx.lineTo(centerX + dirX * 8, centerY + dirY * 8);
    ctx.stroke();
  }

  drawEnemies() {
    if (!GAME.enemies || !GAME.player) return;
    ...
    for (const enemy of GAME.enemies) {
      ...
      if (distance > this.worldRange / 2) continue;
  ...
  drawPickups() {
    if (!GAME.pickups || !GAME.player) return;
    
    const ctx = this.ctx;
    
    for (const pickup of GAME.pickups) {
      const pos = this.worldToMinimap(pickup.x, pickup.y);
      if (!this.isInBounds(pos.x, pos.y)) continue;
  ...
  drawPortal() {
    if (!GAME.portal || !GAME.player) return;
    
    const ctx = this.ctx;
    const pos = this.worldToMinimap(GAME.portal.x, GAME.portal.y);
    ...
    ctx.beginPath();
    ctx.arc(pos.x, pos.y, 3 * pulse, 0, Math.PI * 2);
    ctx.fill();
  }
  ...
  drawInfo() {
    ...
    ctx.fillRect(this.size - 25, 5, 20, 15);
    
    ctx.fillStyle = '#ffffff';
    ctx.font = '10px monospace';
    ctx.textAlign = 'center';
    ctx.fillText('N', this.size - 15, 15);
    
    // Información del bioma
    ctx.fillStyle = 'rgba(0, 0, 0, 0.6)';
    ctx.fillRect(5, this.size - 20, 30, 15);
    
    ctx.fillStyle = '#d4af37';
    ctx.font = '8px monospace';
    ctx.textAlign = 'left';
    ctx.fillText(`B${BIOME + 1}`, 7, this.size - 8);
  }
  ...
  onClick(event) {
    ...
    const scale = this.worldRange / this.size;
    const centerX = this.size / 2;
    const centerY = this.size / 2;
    
    const worldOffsetX = (clickX - centerX) * scale;
    const worldOffsetY = (clickY - centerY) * scale;
    
    const distance = Math.hypot(worldOffsetX, worldOffsetY);
    toast(`Distancia: ${Math.round(distance)}u`);
  }

  update() {
    if (!this.visible || !GAME.running || !GAME.player) return;
...
// Crear instancia del mini mapa
let miniMap = null;

// Inicializar después de que el juego esté listo
function initMiniMap() {
  if (!miniMap && GAME.cache && GAME.player) {
    miniMap = new SimpleMiniMap();
  }
}

// Modificar la función update existente
...
// Inicializar cuando el juego esté listo
const originalBtnStartHandler = document.getElementById('btnStart').onclick;
...
// También intentar inicializar periódicamente
const initChecker = setInterval(() => {
  if (GAME.cache && GAME.player && !miniMap) {
    initMiniMap();
    clearInterval(initChecker);
  }
}, 500);

console.log('Mini mapa simple cargado. Presiona M para mostrar/ocultar.');

/* =========================================================
   SISTEMA DE DASH MEJORADO
   ========================================================= */

// Añadir propiedades de dash al Player
if (typeof Player !== 'undefined') {
  ...
  Player.prototype.update = function(dt) {
    ...
    // Sistema de dash con Shift
    if (!this.dashCd) this.dashCd = 0;
    if (!this.dashSpeed) this.dashSpeed = 1;
    ...
    if (this.dashSpeed > 1) {
      this.dashSpeed -= dt * 0.003; // Decae gradualmente
      if (this.dashSpeed < 1) this.dashSpeed = 1;
    }
    
    // Modificar velocidad base
    const originalSpd = this.spd;
    this.spd = originalSpd * this.dashSpeed;
  };

  Player.prototype.performDash = function() {
    this.dashCd = 2000; // 2 segundos de cooldown
    this.dashSpeed = 2.5; // Velocidad x2.5
    
    // Efectos visuales
    playSfx('light', 0.4, 1.5); // Sonido de dash
    toast('Dash!');
    
    // Partículas de dash (simple)
    if (GAME.particles) {
      for (let i = 0; i < 8; i++) {
...
console.log('Sistema de dash cargado. Mantén SHIFT para dash.');
...
// Integrar partículas en el draw loop
const originalDraw = draw;
function draw() {
  // Llamar función original
  originalDraw();
  
  // Dibujar partículas después
  ctx.save();
  ctx.translate(Math.floor(cv.width/2 - CAMERA.x), Math.floor(cv.height/2 - CAMERA.y));
  drawParticles();
  ctx.restore();
...
// Integrar actualización de partículas
const originalUpdateWithParticles = update;
function update(dt) {
  // Llamar función de update ya modificada
  originalUpdateWithParticles(dt);
  
  // Actualizar partículas
  updateParticles(dt);
}

console.log('Sistema de partículas simple activado.');
